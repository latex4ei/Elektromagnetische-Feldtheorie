...
\end{enumerate}
	...
	\makecell[l]{$\Phi = \iiint\limits_VG(\vec{r},\vec{r}')\rho(\vec{r}')d^3r$\\$\phi$: Greenfunktion lösen für\\ Ergebnis und Orthogonal-\\reihenentwicklung}&
	$\Phi = 0$
	\\\cmrule
	\makecell[l]{inhomogene\\Randwerte} &
	\makecell[l]{Ansatz: $\Phi = \phi + \psi$\\(Randwertprobleme)\\$\psi$: Dirichlet/Neumann RWB,\\$\phi$: Greenfunktion}&
	\makecell[l]{Orthogonal-\\reihenentwicklung}
	\\\cbrule
	\end{tabular*}}
	
	\subsection{$\phi$ bestimmen}
	...
	\begin{enumerate}
		\item $b_n(\vec{r}) = b_1(x)b_2(x)b_3(x) \hat{=} X(x)Y(y)Z(z)$
